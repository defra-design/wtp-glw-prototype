...
router.post('/cookies', function(req, res) {
    if ((req.session.data['agency-officer-v4-cookies-yes-no'] == "yes") || (req.session.data['agency-officer-v4-cookies-yes-no'] == "no")) {
        req.session.data['agency-officer-v4-cookie-status'] = "changed";
        // Reload the same page, "Your cookie settings were saved" messaging will be triggered...
        res.redirect('cookies');
    } else {
        req.session.data['agency-officer-v4-cookie-status'] = "error";
